...
window.countNRooksSolutions = function(n) {
  ...
  var recurse = function(row, rowInd){
    if (rowInd === n){
      solutionCount++;
      return;
    } else {
      for (var colInd = 0; colInd < n; colInd++){
        // console.log(board.get(rowInd)[colInd]);
        board.togglePiece(rowInd, colInd);
        if (!board.hasAnyRooksConflicts()){
          recurse(board.get(rowInd + 1), rowInd + 1);
        }
        board.togglePiece(rowInd, colInd); //MM added
      }
    }
  };

  // Base case: rowCount === n
  // Iterate over rows
  //   For each row,
  //     Place a rook
  //     Recurse over subsequent rows
  //
